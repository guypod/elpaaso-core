...
public class RouteActivationPluginTest {
    ...
    public void should_not_delete_route_if_route_has_not_been_activated_yet() throws Exception {
        Route route = new Route(new RouteUri("host1.mysubdomain.cfapps.elpaaso.net"), "root1", new Space());
    ...
    public void should_return_success_task_on_deletion_success() throws Exception {
        ...
        Assertions.assertThat(status.hasSucceed()).isEqualTo(true);
        Assertions.assertThat(status.getTitle()).isEqualTo("Route <host1.mysubdomain.cfapps.elpaaso.net> has been deleted.");
    }
    ...
    public void should_return_success_task_on_activation_success() throws Exception {
        Route route = new Route(new RouteUri("host1.mysubdomain.cfapps.elpaaso.net"), "root1", new Space());
        Mockito.when(routeRepository.findOne(1)).thenReturn(route);
        Mockito.when(routeActivationService.activate(Mockito.any(Route.class))).thenReturn(new RouteUri("host1.mysubdomain.cfapps.elpaaso.net"));
        ...
        Assertions.assertThat(status.hasSucceed()).isEqualTo(true);
        Assertions.assertThat(status.getTitle()).isEqualTo("Route <host1.mysubdomain.cfapps.elpaaso.net> has been activated.");
    }
}